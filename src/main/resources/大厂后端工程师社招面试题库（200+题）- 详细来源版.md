...
47. **什么是TLAB？**[(参考链接)](https://www.baeldung.com/jvm-thread-local-arena)
   - **全称**：Thread Local Allocation Buffer，线程本地分配缓冲区
   - **目的**：为每个线程分配私有的堆空间，减少多线程分配内存时的同步开销
   - **工作原理**：每个线程预先申请一块堆空间，分配对象时先在TLAB中分配，满后再申请新的TLAB
   - **优势**：提高对象分配效率，减少锁竞争
   - **相关参数**：-XX:+UseTLAB启用TLAB，-XX:TLABSize设置大小

48. **Java对象的创建过程？**[(参考链接)](https://www.baeldung.com/java-object-creation)
   - **类加载检查**：检查类是否已加载，如未加载则执行类加载过程
   - **内存分配**：在Eden区分配内存，使用指针碰撞或空闲列表方式
   - **内存初始化**：将分配的内存空间初始化为零值（不包括对象头）
   - **设置对象头**：设置对象的元数据信息，如哈希码、GC分代年龄、锁标志等
   - **执行init方法**：执行<init>方法，按程序员意愿初始化对象
   - **对象创建完成**：返回对象地址
...
52. **创建线程的几种方式？**[(参考链接)](https://www.baeldung.com/java-thread-start)
   - **继承Thread类**：重写run()方法，创建实例并调用start()
   - **实现Runnable接口**：实现run()方法，传入Thread构造函数
   - **实现Callable接口**：实现call()方法（有返回值），配合FutureTask使用
   - **使用线程池**：通过Executors工厂类创建线程池
   - **使用CompletableFuture**：JDK8新增，支持异步编程
